...
**⚠️ 配置注意：** config.py的writer/reader默认值必须与此设计对齐：
- writer（SVO提取）→ 默认指向本地Qwen3-30B（`localhost:8081`）
- reader（辩证合成）→ 默认指向云端mimo-v2.5-pro（`openrouter`）
- 当前代码默认值与此**相反**，需要在实现时修正。
...
| FUSION_LLM_BASE_URL | http://localhost:8081/v1 | 提取LLM（本地Qwen3-30B） |
| FUSION_LLM_MODEL | local | 提取模型 |
| FUSION_WRITER_BASE_URL | (同FUSION_LLM) | SVO提取（默认=提取LLM） |
| FUSION_READER_BASE_URL | openrouter | 合成LLM（云端mimo-v2.5-pro） |
...
```python
db = sqlite3.connect(db_path, timeout=30.0)  # 30s busy timeout
...
| 故障 | 影响 | 降级策略 |
|------|------|----------|
| Qdrant Cloud不可用 | 写入失败+检索失败 | 写入队列到SQLite，恢复后重放；检索降级到SQLite FTS5 |
